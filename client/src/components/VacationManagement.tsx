...
function getVacationDisplayStatus(vacation: {
  status: string;
  startDate: string;
}) {
  ...
  if (vacation.status === "Aprovada") {
    return startDate > today ? "Programado" : "Concedido";
  }
...
export default function VacationManagement() {
  ...
  const employeeVacationSummary = useMemo(() => {
    return employees.map((employee) => {
      ...
      if (expiredCount > 0) {
        vacationStatus = `${expiredCount} pendente(s)`;
      } else if (nearExpirationCount > 0) {
        vacationStatus = `${nearExpirationCount} a vencer`;
                  ...
                  {viewingEmployeeHistory.map((vacation) => (
                    <tr key={vacation.id} className="border-b border-gray-200">
                      ...
                      <td className="py-4 px-4">
                        <span
                          className={`px-2 py-1 rounded text-sm ${
                            getVacationDisplayStatus(vacation) === "Concedido"
                              ? "bg-green-100 text-green-700"
                              : getVacationDisplayStatus(vacation) === "Programado"
                              ? "bg-cyan-100 text-cyan-700"
                              : getVacationDisplayStatus(vacation) === "Pendente"
                              ? "bg-yellow-100 text-yellow-700"
                              : "bg-red-100 text-red-700"
                          }`}
                        >
                          {getVacationDisplayStatus(vacation)}
                        </span>
                      </td>
                  ...
                  >
                    ...
                    <div className="text-sm space-y-1">
                      ...
                      {alert.isExpired ? (
                        ...
                        <div className="font-medium">
                          Situação: A vencer em {alert.daysUntilExpiration} dias
                        </div>
                      )}
                    </div>
                  </div>
